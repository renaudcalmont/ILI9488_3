...
// Color definitions
#define ILI9488_BLACK       0x000000      /*   0,   0,   0 */
#define ILI9488_NAVY        0x000080      /*   0,   0, 128 */
#define ILI9488_DARKGREEN   0x008000      /*   0, 128,   0 */
#define ILI9488_DARKCYAN    0x008080      /*   0, 128, 128 */
#define ILI9488_MAROON      0x800000      /* 128,   0,   0 */
#define ILI9488_PURPLE      0x800080      /* 128,   0, 128 */
#define ILI9488_OLIVE       0x808000      /* 128, 128,   0 */
#define ILI9488_LIGHTGREY   0xC0C0C0      /* 192, 192, 192 */
#define ILI9488_DARKGREY    0x808080      /* 128, 128, 128 */
#define ILI9488_BLUE        0x0000FF      /*   0,   0, 255 */
#define ILI9488_GREEN       0x00FF00      /*   0, 255,   0 */
#define ILI9488_CYAN        0x00FFFF      /*   0, 255, 255 */
#define ILI9488_RED         0xFF0000      /* 255,   0,   0 */
#define ILI9488_MAGENTA     0xFF00FF      /* 255,   0, 255 */
#define ILI9488_YELLOW      0xFFFF00      /* 255, 255,   0 */
#define ILI9488_WHITE       0xFFFFFF      /* 255, 255, 255 */
#define ILI9488_ORANGE      0xFFA500      /* 255, 165,   0 */
#define ILI9488_GREENYELLOW 0xADFF2F      /* 173, 255,  47 */
//////#define ILI9488_PINK        0x00F81F
...
class ILI9488_t3 : public Print
{
  public:
	...
	void fillScreenHGradient(uint32_t color1, uint32_t color2);
	...
	// Pass 8-bit (each) R,G,B, get back 24-bit packed color
	static uint32_t color888(uint8_t r, uint8_t g, uint8_t b) {
		return ((r & 0xFFFFFF) << 16) | ((g & 0xFFFF) << 8) | ((b & 0xFF) << 0);
	}

	//color888toRGB		- converts 888 format 16 bit color to RGB
	static void color888toRGB(uint32_t color, uint8_t &r, uint8_t &g, uint8_t &b) {
		r = (color & 0xFF0000) >> 16;
		g = (color & 0x00FF00) >> 8;
		b = (color & 0x0000FF) >> 0;
	}
	...
	/*static void color565toRGB14(uint32_t color, int16_t &r, int16_t &g, int16_t &b) {
		r = (color>>2)&0x3E00;
		g = (color<<3)&0x3F00;
		b = (color<<9)&0x3E00;
	}
	...
	void writeRect8BPP(int16_t x, int16_t y, int16_t w, int16_t h, const uint8_t *pixels, const uint32_t * palette );

	// writeRect4BPP - 	write 4 bit per pixel paletted bitmap
	//					bitmap data in array at pixels, 4 bits per pixel
	//					color palette data in array at palette
	//					width must be at least 2 pixels
	void writeRect4BPP(int16_t x, int16_t y, int16_t w, int16_t h, const uint8_t *pixels, const uint32_t * palette );
	
	// writeRect2BPP - 	write 2 bit per pixel paletted bitmap
	//					bitmap data in array at pixels, 4 bits per pixel
	//					color palette data in array at palette
	//					width must be at least 4 pixels
	void writeRect2BPP(int16_t x, int16_t y, int16_t w, int16_t h, const uint8_t *pixels, const uint32_t * palette );
	
	// writeRect1BPP - 	write 1 bit per pixel paletted bitmap
	//					bitmap data in array at pixels, 4 bits per pixel
	//					color palette data in array at palette
	//					width must be at least 8 pixels
	void writeRect1BPP(int16_t x, int16_t y, int16_t w, int16_t h, const uint8_t *pixels, const uint32_t * palette );
	...
	void drawRoundRect(int16_t x0, int16_t y0, int16_t w, int16_t h, int16_t radius, uint32_t color);
	void fillRoundRect(int16_t x0, int16_t y0, int16_t w, int16_t h, int16_t radius, uint32_t color);
	void drawBitmap(int16_t x, int16_t y, const uint8_t *bitmap, int16_t w, int16_t h, uint32_t color);
	void drawChar(int16_t x, int16_t y, unsigned char c, uint32_t color, uint32_t bg, uint8_t size);
 ...
 protected:
	...
	void writedata16_cont(uint16_t d) __attribute__((always_inline)) {
		writedata8_cont((d >> 8) & 0xff);
		writedata8_cont((d >> 0) & 0xff);
	}
	void writecolor_cont(uint32_t d) __attribute__((always_inline)) {
		writedata8_cont((d >> 16) & 0xff);
		writedata8_cont((d >> 8) & 0xff);
		writedata8_cont((d >> 0) & 0xff);
	}
	...
	void writedata16_last(uint16_t d) __attribute__((always_inline)) {
		writedata8_cont((d >> 8) & 0xff);
		writedata8_last((d >> 0) & 0xff);
	}
	void writecolor_last(uint32_t d) __attribute__((always_inline)) {
		writedata8_cont((d >> 16) & 0xff);
		writedata8_cont((d >> 8) & 0xff);
		writedata8_last((d >> 0) & 0xff);
	}
	void HLine(int16_t x, int16_t y, int16_t w, uint32_t color)
	  __attribute__((always_inline)) {
		...
		do { writecolor_cont(color); } while (--w > 0);
	}
	void VLine(int16_t x, int16_t y, int16_t h, uint32_t color)
	  __attribute__((always_inline)) {
		...
		setAddr(x, y, x, y+h-1);
		writecommand_cont(ILI9488_RAMWR);
		do { writecolor_cont(color); } while (--h > 0);
	}
